  ...
  return [
    panel('地球上の推定軌道（正距円筒図法）', {
      ...
      el('div', { class:'note', html:'地上軌跡の向きと緯度だけで軌道傾斜角が決まる。球面三角法から '
        ...
        + '<b>i = ' + f(incObs, 1) + '°</b> となる。90° を少し超える逆行で、これは地球観測衛星が使う'
        + '<b>太陽同期軌道</b>（軌道面が太陽に対して同じ向きを保ち、同じ地方時に同じ場所の上空を通る）の値である。'
        + '傾斜角の低い候補（22.8 / 51.6 / 62.5°）は、この緯度をこの向きに横切れないので、この 1 点だけで外れる。' })
    ]),
    panel('推定されるデブリ（衛星）とのフィッティング', {
      ...
      NS.kv([
        ...
        ['質量', best.c.mass + ' kg'],
        ['決め手', '方位から求めた軌道傾斜角 ' + f(incObs, 1) + '° は太陽同期軌道の値。傾斜角の低い候補は横切れず、残る太陽同期の 2 天体は再突入予報時刻と軌道速度で分かれる'],
